<template>
    <div class="h-screen flex">
        ...
        <div class="flex flex-col m-auto text-center justify-center">
            <h2
                class="my-5 text-xl font-extrabold text-gray-900 dark:text-white md:text-5xl lg:text-6xl"
            >
                <span
                    class="text-transparent bg-clip-text bg-gradient-to-r to-lime-600 from-sky-400"
                    >{{ crypto.name }} - {{ crypto.symbol }}</span
                >
            </h2>
            <h3
            ...
            >
                ...
                <mark class="px-2 text-white bg-lime-500 rounded"
                    >{{ currencyFormatter(crypto.values[0].value) }} €</mark
                >
            </h3>
            <VueApexCharts
                width="700"
...
export default {
    ...
    mounted() {
        ...
        const sortedValues = [...this.crypto.values].sort(
            (a, b) => new Date(a.value_date) - new Date(b.value_date)
        );

        this.options.xaxis.categories = sortedValues.map(
            (item) => item.value_date
        );
        this.series[0].data = sortedValues.map((item) =>
    ...
    components: { SidebarComponent, VueApexCharts, AdminSidebar },
};
</script>
